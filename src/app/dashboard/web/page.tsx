...
export default function WebPage() {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    setStatus("idle");
    try {
      const response = await apiFetch("/analyze/web", {
        method: "POST",
        body: JSON.stringify({ url }),
      });
      if (!response.ok) throw new Error("Error en el servidor de agentes");
      setStatus("success");
      setMessage("Articulo encolado! El agente esta procesando el contenido web.");
      setUrl("");
      loadHistory();
    } catch (err: unknown) {
      setStatus("error");
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        <h1 className="text-3xl font-bold">
          Web <span className="emerald-text-gradient">Scraper</span>
        </h1>
        <p className="text-gray-500 mt-1">
          Extrae y analiza el contenido de cualquier articulo o pagina web.
        </p>
      </div>
        ...
        <form onSubmit={handleAnalyze} className="max-w-3xl mx-auto space-y-6">
          ...
          >
            {loading ? (
              ...
              <>
                <FileText className="w-4 h-4" /> Analizar Articulo
              </>
            )}
          </button>
      ...
      <div className="glass overflow-hidden">
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-lg">Historial de Articulos</h3>
          </div>
        ...
        {histLoading ? (
          ...
          <div className="p-6 text-center text-gray-600 text-sm">
            Los articulos procesados apareceran aqui.
          </div>
        ) : (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-white/5 rounded-lg flex items-center justify-center group-hover:bg-[#10b981]/10 transition-colors">
                    <FileText className="w-4 h-4 text-gray-400 group-hover:text-[#10b981]" />
                  </div>
                  <div>
