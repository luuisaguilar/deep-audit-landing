...
export default function ChefPage() {
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("success");
      setMessage("¡Receta encolada! El Chef IA está extrayendo los ingredientes y pasos.");
      setUrl("");
    } catch (err: any) {
      setStatus("error");
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        <h1 className="text-3xl font-bold">Chef <span className="emerald-text-gradient">IA</span></h1>
        <p className="text-gray-500 mt-1">Extrae recetas estructuradas de videos de cocina en YouTube.</p>
      </div>

      <div className="glass p-8">
        <form onSubmit={handleAnalyze} className="max-w-3xl mx-auto space-y-6">
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-widest text-gray-500">URL del Video de Cocina</label>
      ...
      <div className="glass overflow-hidden">
        <div className="p-6 border-b border-white/5 flex items-center gap-2">
          ...
          <h3 className="font-bold text-lg">Recetas Extraídas</h3>
        </div>
        <div className="p-6 text-center text-gray-600 text-sm">
          Las recetas guardadas en tu Vault aparecerán aquí.
        </div>
      </div>
    </div>
  );
